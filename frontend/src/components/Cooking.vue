    ...
    <v-card style="width:450px;" outlined>
        ...
        <v-card-actions>
            ...
            <v-btn
                    ...
                    color="deep-purple lighten-2"
                    text
                    @click="openAcceptorreplace"
            >
                Acceptorreplace
            </v-btn>
            <v-dialog v-model="acceptorreplaceDiagram" width="500">
                <AcceptorreplaceCommand
                        @closeDialog="closeAcceptorreplace"
                        @acceptorreplace="acceptorreplace"
                ></AcceptorreplaceCommand>
            </v-dialog>
            <v-btn
                    v-if="!editMode"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async acceptorreplace(params) {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['acceptorreplace'].href), params)
            ...
            openAcceptorreplace() {
                this.acceptorreplaceDiagram = true;
            },
            closeAcceptorreplace() {
                this.acceptorreplaceDiagram = false;
            },
            async start() {
                try {
                    if(!this.offline) {
